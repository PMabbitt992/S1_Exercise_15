...
window.onload = function () {
      ...
      //write the field values to hte order form
...
window.addEventListener("load", function () {
      ...
      document.getElementById("getMonth").onchange = validateMonth;
      document.getElementById("getYear").onchange = validateYear;
});
...
function validateCredit() {
      ...
      if (creditCard.validity.valueMissing) {
            creditCard.setCustomValidity("Select credit card");
      } else {
...
function luhn(idNum) {
      var string1 = "";
      var string2 = "";
      //retrieve odd number digits
      for (var i = idNum.length - 1; i >= 0; i -= 2) {
            string1 += idNum.charAt(i);
      }
      //retrieve even digits and double 
      for (var i = idNum.length - 2; i >= 0; i -= 2) {
            string2 += 2 * idNum.charAt(i);
      }
      //return whether the sum of digits is divisible by ten
